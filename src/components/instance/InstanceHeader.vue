<template>
  <div id="instanceHeader" class="flex borbox noselect">
    ...
    <div class="header-side-opts flex">
      <div
        class="flex flex-ai flex-jcc pointer relative"
        v-for="item in sideOpts"
        :key="item.name"
        :title="headerLang.sideOpts[item.name]"
        @click="handleSideOpts(item.name)"
      >
        <div
          class="badge-dot"
          v-if="!isLogRead && item.name === 'versionLogs'"
        ></div>
...
export default {
  ...
  methods: {
    ...
    async compareVersions() {
      ...
      const { tag_name: githubV } = await get(
        "/githubApi/repositories/190842308/releases/latest"
      );
